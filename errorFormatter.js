...
function formatError(error, severity) {
    ...
    if (file == '--macro') {
        ...
        file = 'unknown location (probably generated by an initialization macro)';
    } else {
        ...
        if (!fs.existsSync(file)) {
            ...
            if (severity === 'warning') {
                extras = [
                    displaySource(src, error.positions, '', 5, 0, 2),
                    ''
                ];
            } else {
                extras = [
    ...
    if (error.contextErrors) {
        error.contextErrors.forEach(error => ret.push(formatSubError(error, severity)));
    }
...
function removeLoaders(file) {
    ...
    // TODO: use a parameter for (optional) prefix
    if (path.isAbsolute(filePath)) return path.normalize(filePath);
    return path.normalize(
        filePath
        // `/git/herakles/herakles-portal/build/env/portal/${filePath}`
    );
}
...
function format(errors, type, forwardOtherErrors) {
    let stdout;
    let formattedErrors = [];

    errors.sort((e1, e2) => {
        const i1 = (e1.webpackError && e1.webpackError.error && e1.webpackError.error.index) || -1;
        const i2 = (e2.webpackError && e2.webpackError.error && e2.webpackError.error.index) || -1;
    ...
    errors.forEach(error => {
        switch (error.type) {
            case 'Haxe Error':
                formattedErrors.push(formatError(error, 'error'));
                break;

            case 'Haxe Warning':
                formattedErrors.push(formatError(error, 'warning'));
                break;
            ...
            default:
                if (forwardOtherErrors) formattedErrors.push(error.message);
        }
    });
    ...
    errors.forEach(error => {
        ...
        if (isSubError(error, previous)) {
            ...
            if (!previous.contextErrors) {
                ...
                // let subAdded = false;
                // const len = previous.contextErrors.length;
                // for (let i = len - 1; i >= 0; i--) {
                //     const previousSub = previous.contextErrors[i];
                //     const prevKey = flattenPosition(previousSub);

                //     if (key === prevKey) {
                //         if (!previousSub.message.split('\n').find(function(l) {
                //             return l.trim() === error.message
                //         })) {
                //             previousSub.message += '\n       ' + error.message;
                //         }
                ...
                //     }
                // }
                // if (!subAdded) previous.contextErrors.push(error);
